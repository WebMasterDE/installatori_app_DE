...
class SaveDataProvider extends ChangeNotifier {
  ...
  Future<bool> saveAppartamentoToLocal(int idAnaCondominio, int idAppartamento) async {
    ...
    if (appartamentoTemp != null) {
      AppartamentoModel appartamento =
          AppartamentoModel.fromJson(jsonDecode(appartamentoTemp));
      ...
      if (condominiString != null) {
        List<CondominioModel> condominiList =
            List.from(jsonDecode(condominiString)).map((condominio) {
  ...
  Future<bool> saveCondominio(CondominioModel condominio) async {
    ...
    if (appartamentiToRemove.isNotEmpty) {
      ...
      if (condominiString != null) {
        List<CondominioModel> condominiList =
            List.from(jsonDecode(condominiString))
                .map((c) => CondominioModel.fromJson(c))
                .toList();
  ...
  Future<bool> saveAppartamentoAndStrumenti(
      AppartamentoModel appartamento) async {
    List<File> fileAppartamento = [];

    if (appartamento.pathUploadImage != null) {
      fileAppartamento.add(File(appartamento.pathUploadImage!));
      appartamento.pathUploadImage =
          "appartamento_${idAnaCondominio}_${getTimestampFromFilename(appartamento.pathUploadImage!)}.png";
    }
    ...
    if (appartamento.riscaldamento != null) {
      if (appartamento.riscaldamento!.ripartitori != null) {
        ...
        for (var ripartitore in appartamento.riscaldamento!.ripartitori!) {
          ...
          ripartitore.pathImage =
              "riscaldamento_${idAnaCondominio}_${id}_${getTimestampFromFilename(ripartitore.pathImage!)}.png";
          id++;
        }
      }
    }
    ...
    if (appartamento.raffrescamento != null) {
      if (appartamento.raffrescamento!.ripartitori != null) {
        ...
        for (var ripartitore in appartamento.raffrescamento!.ripartitori!) {
          ...
          ripartitore.pathImage =
              "raffrescamento_${idAnaCondominio}_${id}_${getTimestampFromFilename(ripartitore.pathImage!)}.png";
          id++;
        }
      }
    }
    ...
    if (appartamento.acquaCalda != null) {
      if (appartamento.acquaCalda!.ripartitori != null) {
        ...
        for (var ripartitore in appartamento.acquaCalda!.ripartitori!) {
          ...
          ripartitore.pathImage =
              "acquaCalda_${idAnaCondominio}_${id}_${getTimestampFromFilename(ripartitore.pathImage!)}.png";
          id++;
        }
      }
    }
    ...
    if (appartamento.acquaFredda != null) {
      if (appartamento.acquaFredda!.ripartitori != null) {
        ...
        for (var ripartitore in appartamento.acquaFredda!.ripartitori!) {
          ...
          ripartitore.pathImage =
              "acquaFredda_${idAnaCondominio}_${id}_${getTimestampFromFilename(ripartitore.pathImage!)}.png";
          id++;
        }
      }
    }
    ...
    dynamic result = await ApiRequests.sendMultipartAuthRequest(
        'condominio/$idAnaCondominio/appartamenti/$idAppartamento',
        'POST',
        {'appartamento': appartamento.toJson()},
        files);
